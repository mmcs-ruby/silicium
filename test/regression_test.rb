...
class RegressionTest < MiniTest::Test
  ...
  def test_polynomial_plot_scaling1
    ...
    assert_equal_arrays result[0].map{|x, _| x}, [0.03125, -0.03125, -0.125, -0.25, 0.0625, 0.0625, 0.03125, -0.03125, -0.40625]
  end

  def test_polynomial_plot_scaling2
    result = Regression::PolynomialRegressionByGradientDescent::feature_scaled_plot(@@pol_plot2, 3)
    p result
    assert_equal_arrays result[0].map{|x, _| x}, [0.17, 0.097, 0.05, 0.023, 0.01, 0.005, 0.002, -0.005, -0.022, -0.055, -0.11]
  end
end