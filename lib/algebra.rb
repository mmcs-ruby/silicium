...
module Silicium
  ...
  require_relative 'polynom_real_roots_sup'
  ##
  ...
  module Algebra
    ...
    module PolynomRootsReal
      ...
      def step_up_loop(arr_pack)
        ...
        root_dif[level][cur_root_count[level]] = binary_root_finder(
            level, edge_neg, edge_pos, cf_dif[level]
        )
        cur_root_count[level] += 1
      end
      ...
      def form_right(args_pack)
        ...
        edge_right = i == cur_root_count[level-1] ? major : root_dif[level - 1][i]# changed level on level - 1
